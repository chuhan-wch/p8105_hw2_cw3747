...
```
When merging, some zipcodes appeared in multiple boroughs (e.g., 10463 in both Bronx and Manhattan). To avoid artificial inflation of the number of observations due to many-to-many matches, I retained one unique mapping per zipcode. This ensures each zipcode has a single geographic identifier while preserving the full rental time series from Zillow. The resulting tidy dataset contains **`r format(n_obs, big.mark = ",")`** observations across **`r n_zip`** unique zipcodes and **`r n_nbhd`** neighborhoods.
